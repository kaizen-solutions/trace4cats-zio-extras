...
final case class ElementTracerMap(
  ...
  def cleanup(exit: Exit[Nothing, Any], strategy: ExecutionStrategy = ExecutionStrategy.Sequential): UIO[Unit] = {
    def completePromise(p: Promise[Nothing, Unit]): UIO[Boolean] = exit match {
      case Exit.Success(_)     => p.succeed(())
      case Exit.Failure(cause) => p.refailCause(cause)
    }

    strategy match {
      case ExecutionStrategy.Sequential =>
        ZIO.foreachDiscard(spanCloseMap.values().asScala)(completePromise) *> openScope.close(exit)

      case ExecutionStrategy.Parallel =>
        ZIO.foreachParDiscard(spanCloseMap.values().asScala)(completePromise) *> openScope.close(exit)

      case ExecutionStrategy.ParallelN(n) =>
        ZIO
          .foreachParDiscard(spanCloseMap.values().asScala)(completePromise)
          .withParallelism(n) *> openScope.close(exit)
    }
  }
}
object ElementTracerMap {
  ...
  def make(openScope: Scope.Closeable, capacity: Int = 128): UIO[ElementTracerMap] =
    ZIO
