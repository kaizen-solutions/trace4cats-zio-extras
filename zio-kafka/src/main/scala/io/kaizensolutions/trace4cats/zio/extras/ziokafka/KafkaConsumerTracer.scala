...
object KafkaConsumerTracer {
  ...
  private implicit def longAttributeValue(value: Long): AttributeValue.LongValue = AttributeValue.LongValue(value)

  private def extractTraceHeaders[K, V](in: CommittableRecord[K, V]): TraceHeaders = {
    TraceHeaders.of(
  ...
  def traceConsumerStream[R, K, V](
    ...
    spanNameForElement: CommittableRecord[K, V] => String = (_: CommittableRecord[K, V]) => s"kafka-receive"
  ): ZStream[R, Throwable, Spanned[CommittableRecord[K, V]]] =
    tracer
      .traceEachElement(extractTraceHeaders[K, V], spanNameForElement, SpanKind.Consumer)(stream)
      .mapZIOWithTracer(tracer, "kafka-consumer") { comm =>
        ...
        //                  val currentSpan = tracer.retrieveCurrentSpan
